...
export default function HolidayManager() {
  ...
  const updateTagMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries(['holidays']); toast.success('Tag updated'); },
  });

  const deleteMutation = useMutation({
    mutationFn: (id) => base44.entities.Holiday.delete(id),
    onSuccess: () => { queryClient.invalidateQueries(['holidays']); toast.success('Holiday removed'); },
  });
  ...
  const handleAdd = () => {
    ...
    createMutation.mutate({ name, startDate, endDate: endDate || startDate, category, tag, order: 0 });
  };

  return (
    <div className="space-y-4">
      <div className="flex flex-wrap gap-3">
        <Button onClick={() => setShowAdd(!showAdd)} size="sm" className="bg-gray-900 hover:bg-gray-800">
          <Plus className="w-4 h-4 mr-2" /> Add Holiday
        </Button>
        {holidays.length === 0 && (
          <Button variant="outline" size="sm" onClick={seedHolidays}>
            <CalendarDays className="w-4 h-4 mr-2" /> Load Defaults
          </Button>
        )}
      </div>
        ...
        <Card className="border-gray-200">
          <CardContent className="p-5 space-y-4">
            ...
            <div className="flex gap-2">
              <Button onClick={handleAdd} disabled={createMutation.isPending} size="sm" className="bg-gray-900 hover:bg-gray-800">
            ...
            <Card key={h.id} className="border-gray-100">
              <CardContent className="p-4 flex items-center justify-between gap-3">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <Badge variant="outline" className="text-[10px] capitalize ml-1">{h.category}</Badge>
                </div>
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  <Button variant="ghost" size="icon" className="h-8 w-8 text-gray-400 hover:text-red-500" onClick={() => deleteMutation.mutate(h.id)}>
